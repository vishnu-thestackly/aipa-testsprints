...
function getListMetrics(availableHeight, itemCount) {
  ...
  const slotsFromSpace = Math.floor(
    (availableHeight + CHAT_ITEM_GAP) / slotHeight
  );
  ...
  return {
    listHeight,
    needsScroll: itemCount > visibleSlots,
  };
}
...
export default function SidebarChatHistory() {
  ...
  const [listHeight, setListHeight] = useState(
    MIN_VISIBLE * CHAT_ITEM_HEIGHT +
      (MIN_VISIBLE - 1) * CHAT_ITEM_GAP
  );
  ...
  useLayoutEffect(() => {
    ...
    scrollParent?.addEventListener("scroll", updateListHeight, {
      passive: true,
    });
  ...
  return (
    <div
      ref={rootRef}
      className="w-full flex flex-col min-h-0 select-none"
    >
      {/* Divider */}
      <div className="w-full border-t border-[#E5E7EB] my-4 lg:my-5" />
      ...
      <div className="flex items-center justify-between px-1 mb-[14px]">
        ...
        <button
          className="text-[#586D93] hover:text-[#4866F6] transition-colors duration-200 focus:outline-none cursor-pointer"
        >
          <svg
            width="18"
            height="18"
