...
useEffect(() => {
  console.log("Revenue State Updated:", revenueData);
}, [revenueData]);
...
const loadTransactions = async () => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      setRows(data.items || data.results || data.data || []);
    }
  } catch (error) {
    console.error(error);
  }
...
const handleViewTransaction = async (paymentId) => {
  try {
    const data = await getTransactionDetail(paymentId);
...
const loadKpis = async () => {
  try {
    const data = await getTransactionKpis();
...
className="w-full h-[34px] border border-[#D9D9D9] rounded-[10px] px-4 flex items-center justify-between bg-white text-[14px] text-[#8D99AE] cursor-pointer"  >
  ...
  {/* Export */}
<button onClick={handleExport} className="w-full md:w-[100px] lg:w-full xl:w-[130px] h-[34px] whitespace-nowrap bg-[#4866F6] rounded-[8px] text-white text-[14px] font-medium flex items-center justify-center gap-2 cursor-pointer">
  Export
...
<table className="w-full min-w-[1100px] table-fixed">
  ...
  <th className="w-[100px] h-[62px] px-5 text-center text-[14px] font-medium text-[#3D3D3D]">Action</th>
      ...
      <tbody>
        {paginatedRows.map((r) => (
          <tr key={r.payment_id} className="border-b border-[#E5E5E5]">
            <td className="h-[52px] px-5 text-[14px] text-[#5A6B95]">{r.payment_id}</td>
...
>
  ...
  {r.status}
            ...
            <td className="h-[52px] px-5">
<div className="flex justify-center cursor-pointer" onClick={() => handleViewTransaction(r.payment_id)}>
              {/* <td className="p-3"> */}
...
<div className="text-left sm:text-right">
          ...
          <span className="ml-1 text-[#8B97AC]">{selectedTransaction?.date
    ...
    : "-"}</span>
        ...
        <div>
          ...
          <span className="ml-1 text-[#8B97AC]">{selectedTransaction?.user_name}</span>
        </div>

<div className="text-left sm:text-right">
              <span className="font-medium text-[#3D3D3D]">Payment Method:</span>
          <span className="ml-1 text-[#8B97AC]">{selectedTransaction?.payment_method}</span>
        </div>
      </div>
